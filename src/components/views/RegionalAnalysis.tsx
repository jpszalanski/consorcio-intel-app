...
import { AppDataStore, RegionalBacenData } from '../../types';
import { Map as MapIcon, Users, AlertTriangle, Database } from 'lucide-react';
...
export const RegionalAnalysis: React.FC<Props> = ({ data }) => {
  ...
  const aggregatedData = useMemo<RegionalBacenData[]>(() => {
    if (!data.regional) return [];
    
    const ufMap = new Map<string, RegionalBacenData>();

    data.regional.forEach(item => {
      // Normalização básica
      ...
      current.activeContemplatedBid += (item.activeContemplatedBid || 0);
      current.activeContemplatedLottery += (item.activeContemplatedLottery || 0);
      current.activeNonContemplated += (item.activeNonContemplated || 0);
      current.dropoutContemplated += (item.dropoutContemplated || 0);
      current.dropoutNonContemplated += (item.dropoutNonContemplated || 0);
      current.newAdhesionsQuarter += (item.newAdhesionsQuarter || 0);
      current.totalActive += (item.totalActive || 0);
    });
